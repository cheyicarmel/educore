    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col items-center text-center gap-3">
                ...
                <div>
                    ...
                    <span class="inline-flex items-center gap-1 mt-2 px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full uppercase">
                        <span class="w-1.5 h-1.5 rounded-full bg-primary"></span>Enseignant
                    </span>
                </div>
            </div>

            {{-- Carte classes & matières --}}
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                <h3 class="text-sm font-bold text-navy-900 mb-4 flex items-center gap-2">
                    ...
                    Mes attributions — {{ $anneeActive?->libelle }}
                </h3>
                @if($attributions->isEmpty())
                <p class="text-xs text-slate-400 text-center py-4">Aucune attribution cette année.</p>
                @else
                <div class="space-y-2">
                    @foreach($attributions as $attr)
                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                        <div>
                            <p class="text-xs font-bold text-navy-900">{{ $attr->classe->nom }}</p>
                            <p class="text-[11px] text-navy-700">{{ $attr->matiere->nom }}</p>
                        </div>
                        @if($attr->est_prof_principal)
                        <span class="px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] font-bold rounded-full">Principal</span>
                        @endif
                    </div>
                    @endforeach
                </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <form method="POST" action="{{ route('enseignant.profil.update') }}" class="p-5 space-y-4">
                    @csrf
                    @method('PUT')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="password" id="password"
                                    ...
                                    required/>
                                <button type="button" onclick="toggleVisiblite('password', 'eye1')"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" name="password_confirmation" id="password_confirmation"
                                    class="w-full px-4 py-2.5 pr-11 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-navy-900 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"
                                    required/>
                                <button type="button" onclick="toggleVisiblite('password_confirmation', 'eye2')"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-navy-700 transition-colors">
...
@section('scripts')
    <script>
    function toggleVisiblite(inputId, eyeId) {
