...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {{-- Sélecteur d'année --}}
        @if($annees->count() > 1)
        <form method="GET" action="{{ route('eleve.bulletins') }}">
    ...
    <div class="flex items-start gap-4 p-5 bg-rose-50 border border-rose-200 rounded-2xl">
        ...
        <div>
            ...
            @if($estAnneeTerminee)
            <p class="text-sm text-rose-600 mt-1">Vous ne pouvez pas accéder à vos anciens bulletins tant que vous n'avez pas soldé l'intégralité de vos frais de scolarité de l'année en cours. Solde restant : <strong>{{ number_format($soldeRestant, 0, ',', ' ') }} FCFA</strong>.</p>
            @else
