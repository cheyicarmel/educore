...
@section('content')
                    ...
                    <tr class="hover:bg-slate-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openModal('modal-coefficients-{{ $matiere->id }}')"
                                    class="p-1.5 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Coefficients">
            ...
            <div class="p-4 space-y-3">
                ...
                <div class="flex items-center gap-2 pt-1">
                    <button onclick="openModal('modal-coefficients-{{ $matiere->id }}')"
                        ...
                        <span class="material-symbols-outlined text-sm">percent</span>Coefficients
                    </button>
                    <button onclick="openModal('modal-edit-{{ $matiere->id }}')"
    ...
    @foreach($matieres as $matiere)
        ...
        {{-- Modal Coefficients --}}
        <div id="modal-coefficients-{{ $matiere->id }}" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/50" onclick="closeModal('modal-coefficients-{{ $matiere->id }}')"></div>
            <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
                <div class="p-5 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10">
                    <div>
                        <h3 class="text-base font-bold text-navy-900">Coefficients — {{ $matiere->nom }}</h3>
                        @if($anneeActive)
                        <p class="text-xs text-navy-700 mt-0.5">Année active : {{ $anneeActive->libelle }}</p>
                        @endif
                    </div>
                    <button onclick="closeModal('modal-coefficients-{{ $matiere->id }}')" class="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
                        <span class="material-symbols-outlined text-xl">close</span>
                    </button>
                </div>
                <form method="POST" action="{{ route('admin.matieres.coefficients', $matiere) }}" class="p-5">
                    @csrf
                    @php $classesMatiere = $classesByMatiere[$matiere->id] ?? collect(); @endphp
                        @if($classesMatiere->isEmpty())
                            <div class="py-8 text-center">
                                <span class="material-symbols-outlined text-slate-300 text-4xl">school</span>
                                <p class="text-sm text-slate-400 font-semibold mt-2">Cette matière n'est assignée à aucune classe pour l'année active.</p>
                            </div>
                        @else
                        <div class="space-y-3">
                            @foreach($classesMatiere as $classe)
                                @php $coeff = $coefficients[$matiere->id][$classe->id]->coefficient ?? 1; @endphp
                                <div class="flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-xl">
                                    <div>
                                        ...
                                        <p class="text-xs text-navy-700">{{ ucfirst($classe->cycle) }} cycle</p>
                                    </div>
                                    <div class="flex items-center gap-2 shrink-0">
                                        <label class="text-xs font-semibold text-navy-700">Coeff.</label>
                                        <input type="number" name="coefficients[{{ $classe->id }}]" value="{{ $coeff }}"
                                            min="1" max="10"
                                            class="w-16 px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-center font-bold text-navy-900 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all"/>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <div class="flex items-center gap-3 pt-5">
                            <button type="button" onclick="closeModal('modal-coefficients-{{ $matiere->id }}')" class="flex-1 py-2.5 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Annuler</button>
                            <button type="submit" class="flex-1 py-2.5 text-sm font-bold text-white bg-primary hover:bg-primary/90 rounded-xl transition-colors">Enregistrer</button>
                        </div>
                    @endif
                </form>
