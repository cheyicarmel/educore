            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        ...
                        @elseif($annee->estAVenir())
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold text-orange-700 bg-orange-100 rounded-full uppercase">
                            <span class="w-1.5 h-1.5 rounded-full bg-orange-500"></span>À venir
                        </span>
                        @else
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            {{-- Modifier uniquement si pas terminée --}}
                            @if(!$annee->estTerminee())
                            ...
                            <button onclick="openModal('modal-status-{{ $annee->id }}')"
                                class="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Terminer l'année">
                            ...
                            @endif
                            {{-- Supprimer uniquement si pas active --}}
                            @if(!$annee->estActive())
    ...
    <div class="md:hidden divide-y divide-slate-100">
        ...
        <div class="p-4 space-y-3">
            ...
            <div class="flex items-center gap-2 pt-1">
                @if(!$annee->estTerminee())
                <button onclick="openModal('modal-edit-{{ $annee->id }}')"
                    class="flex-1 flex items-center justify-center gap-1 py-2 text-xs font-semibold text-primary bg-primary/10 hover:bg-primary/20 rounded-lg transition-colors">
                    <span class="material-symbols-outlined text-sm">edit</span>Modifier
                </button>
                @endif
                @if($annee->estActive())
                <button onclick="openModal('modal-status-{{ $annee->id }}')"
                    class="flex-1 flex items-center justify-center gap-1 py-2 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg transition-colors">
                    <span class="material-symbols-outlined text-sm">event_busy</span>Terminer
                </button>
                @elseif($annee->estAVenir())
                <button onclick="openModal('modal-status-{{ $annee->id }}')"
                    class="flex-1 flex items-center justify-center gap-1 py-2 text-xs font-semibold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-colors">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div class="p-6 text-center">
            ...
            <div class="w-14 h-14 rounded-full bg-rose-100 flex items-center justify-center mx-auto mb-4">
                <span class="material-symbols-outlined text-rose-500 text-2xl">event_busy</span>
            </div>
            <h3 class="text-base font-bold text-navy-900 mb-2">Terminer cette année ?</h3>
            <p class="text-sm text-navy-700 mb-6">L'année <strong>{{ $annee->libelle }}</strong> sera marquée comme terminée. Cette action est irréversible.</p>
            @else
            ...
            <p class="text-sm text-navy-700 mb-6">L'année <strong>{{ $annee->libelle }}</strong> deviendra l'année académique active.</p>
            @endif
            <div class="flex items-center gap-3">
                <button type="button" onclick="closeModal('modal-status-{{ $annee->id }}')"
                ...
                <form method="POST" action="{{ route('admin.annees.status', $annee) }}" class="flex-1">
                    ...
                    @method('PATCH')
                    <button type="submit"
                        class="w-full py-2.5 text-sm font-bold text-white {{ $annee->estActive() ? 'bg-rose-500 hover:bg-rose-600' : 'bg-emerald-500 hover:bg-emerald-600' }} rounded-xl transition-colors">
                        Confirmer
                    </button>
                </form>
            </div>
...
<div id="modal-delete-{{ $annee->id }}" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div class="p-6 text-center">
            ...
            <div class="flex items-center gap-3">
                <button type="button" onclick="closeModal('modal-delete-{{ $annee->id }}')"
                    class="flex-1 py-2.5 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Annuler</button>
