...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Infos élève */
        .eleve-band { width: 100%; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; margin-bottom: 12px; border-left: 4px solid #2b6cee; }
        ...
        .notes-table thead tr { background: #1e40af; }
        .notes-table thead th { padding: 6px 8px; font-size: 8px; font-weight: 700; color: white; text-transform: uppercase; border: 1px solid #1d4ed8; text-align: center; }
        .notes-table thead th.left { text-align: left; }
        .notes-table tbody tr:nth-child(even) { background: #f8fafc; }
        .notes-table tbody tr:nth-child(odd) { background: #ffffff; }
        .notes-table tbody td { padding: 6px 8px; font-size: 9px; border: 1px solid #e2e8f0; vertical-align: middle; }
        .notes-table tbody td.center { text-align: center; }
        .notes-table tbody td.moy-cell { background: #eff6ff; text-align: center; font-weight: 800; border-left: 2px solid #93c5fd; border-right: 2px solid #93c5fd; }
        .notes-table tfoot td { padding: 7px 8px; font-size: 9px; font-weight: 800; background: #1e3a8a; color: white; border: 1px solid #1d4ed8; text-align: center; }
        .notes-table tfoot td.left { text-align: left; }
        ...
        /* Résumé */
        .resume-table { width: 100%; border-collapse: separate; border-spacing: 6px; margin-bottom: 12px; }
        .resume-cell { padding: 10px; border-radius: 6px; text-align: center; vertical-align: middle; }
        .resume-label { font-size: 7px; font-weight: 700; text-transform: uppercase; display: block; margin-bottom: 4px; }
        .resume-value { font-size: 16px; font-weight: 800; display: block; }
        .resume-sub { font-size: 7px; display: block; margin-top: 2px; }

        .mention-badge { font-size: 9px; font-weight: 800; padding: 3px 10px; border-radius: 10px; }
...
<div class="page">
    ...
    <table class="notes-table">
        <thead>
            <tr>
                <th class="left" style="width:25%;">Matière</th>
                @if(!$estPremierCycle)
                <th style="width:8%;">Coeff.</th>
                @endif
                <th style="width:12%;">Moy. Interro.</th>
                <th style="width:12%;">Moy. Matière</th>
                @if(!$estPremierCycle)
                <th style="width:12%;">Moy. × Coeff.</th>
                @endif
        ...
        <tbody>
            ...
            <tr>
                <td><strong>{{ $dm['matiere'] }}</strong></td>
                @if(!$estPremierCycle)
                <td class="center">{{ $dm['coefficient'] ?? '—' }}</td>
                @endif
                <td class="center">
                    @if($dm['moyenne_interrogations'] !== null)
                    <span class="{{ $dm['moyenne_interrogations'] >= 10 ? 'ok' : 'ko' }}">
                        {{ number_format($dm['moyenne_interrogations'], 2) }}
                    </span>
                    @else —
                    @endif
                </td>
                <td class="moy-cell">
                    ...
                    <span class="{{ $dm['moyenne_generale'] >= 10 ? 'ok' : 'ko' }}">
                ...
                <td class="center">
                    ...
                    @php
                        ...
                        $app = $v >= 16 ? 'Excellent' : ($v >= 14 ? 'Très bien' : ($v >= 12 ? 'Bien' : ($v >= 10 ? 'Assez bien' : ($v >= 8 ? 'Passable' : 'Insuffisant'))));
                    @endphp
                    <span style="font-size:8px; color:#475569;">{{ $app }}</span>
                    @else —
                    @endif
                </td>
            </tr>
        ...
        <tfoot>
            <tr>
                <td class="left" colspan="{{ $estPremierCycle ? 1 : 2 }}">MOYENNE GÉNÉRALE DU SEMESTRE</td>
                <td colspan="{{ $estPremierCycle ? 2 : 3 }}">
                    @if($moyenne !== null)
        ...
        <tr>
            <td>
                <div class="resume-cell" style="background:#eff6ff; border:1px solid #bfdbfe;">
                    <span class="resume-label" style="color:#1d4ed8;">Moyenne</span>
                    <span class="resume-value" style="color:{{ ($moyenne ?? 0) >= 10 ? '#15803d' : '#dc2626' }};">
                        {{ $moyenne !== null ? number_format($moyenne, 2) : '—' }}<span style="font-size:10px;">/20</span>
                    </span>
                </div>
            </td>
            <td>
                <div class="resume-cell" style="background:#f0fdf4; border:1px solid #bbf7d0;">
                    ...
                    <span class="resume-value" style="color:#15803d;">
                        {{ $rang ?? '—' }}<span style="font-size:10px;">/ {{ $effectif }}</span>
                    </span>
                </div>
            </td>
            <td>
            ...
            <td>
                <div class="resume-cell" style="background:#f8fafc; border:1px solid #e2e8f0;">
                    ...
                    <span class="resume-value" style="color:#1e40af;">
                        {{ number_format($moyenneClasse, 2) }}<span style="font-size:10px;">/20</span>
                    </span>
                </div>
            </td>
        </tr>
    ...
    <div style="border:1px solid #e2e8f0; border-radius:6px; padding:8px 12px; margin-bottom:14px; background:#fafafa;">
        <p style="font-size:7px; font-weight:700; color:#4c669a; text-transform:uppercase; margin-bottom:6px;">Appréciation du Professeur Principal</p>
